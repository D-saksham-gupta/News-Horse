       ...
       const handlePre= async ()=>{
        ...
        return (
            <div className="container my-3">
                <h1 className="text-center my-3">NewsHorse - Top Headlines from {capitalizeFirst(props.category)} News</h1>
               {loading && <Spinner/>}
                <div className="row">
                ...
                <button  disabled={page<=1} type="button" className="btn btn-dark" style={{backgroundColor:'#7952b3'}} onClick={handlePre}>&larr; Previous</button>
                <button disabled={page + 1 > Math.ceil(totalResults/props.pageSize)} type="button" className="btn btn-dark" style={{backgroundColor:'#7952b3'}}  onClick={handleNext}>Next &rarr;</button>
                </div>
